...
class RoleSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        token = self.values[0]
        resolved = await service.resolve_binding(self.message_id, "select", token)
        if not resolved.role_id or not resolved.bundle_key or not resolved.language:
        ...
        role = interaction.guild.get_role(resolved.role_id)
        if role is None:
...
class RoleButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        resolved = await service.resolve_binding(self.message_id, "button", self.token)
        if not resolved.role_id or not resolved.bundle_key or not resolved.language:
            await interaction.response.send_message("Configuração não encontrada.", ephemeral=True)
            return
        ...
        if not allowed:
            await interaction.response.send_message(
                "Escolha duplicada entre línguas ignorada.", ephemeral=True
            )
            return
...
class ReactionRolesCog(commands.Cog):
    ...
    async def rr_create_reaction(
        ...
        msg = "Reação vinculada ao cargo." if ok else "Mensagem não registrada em bundle."
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        resolved = await service.resolve_binding(payload.message_id, "reaction", emoji_token)
        if not resolved.role_id or not resolved.bundle_key or not resolved.language:
            return
        ...
        role = guild.get_role(resolved.role_id)
        if role:
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        ...
        resolved = await service.resolve_binding(payload.message_id, "reaction", str(payload.emoji))
        if resolved.role_id:
            role = guild.get_role(resolved.role_id)
            if role:
                await member.remove_roles(role, reason="LunarBot reaction role removed")
